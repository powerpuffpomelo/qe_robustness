...
from examples.word_level.wmt_2020.en_zh.microtransquest_config_robust_train_memory import TRAIN_PATH, TRAIN_SOURCE_FILE, \
    TRAIN_SOURCE_TAGS_FILE, \
    TRAIN_TARGET_FILE, \
    TRAIN_TARGET_TAGS_FLE, TRAIN_ROBUST_IDX_FILE, \
    MODEL_TYPE, MODEL_NAME, microtransquest_config, TEST_PATH, TEST_SOURCE_FILE, \
...
model = MicroTransQuestModel(MODEL_TYPE, MODEL_NAME, labels=["OK", "BAD"], args=microtransquest_config, use_cuda=True,)
...
for i in range(microtransquest_config["n_fold"]):
    ...
    if microtransquest_config["evaluate_during_training"]:
        ...
        #raw_eval.to_csv("/home/yanym/save/transquest/raw_eval.csv")
        model = MicroTransQuestModel(MODEL_TYPE, MODEL_NAME, labels=["OK", "BAD"], args=microtransquest_config, use_cuda=True,)
