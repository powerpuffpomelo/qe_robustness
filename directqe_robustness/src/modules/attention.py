...
class MultiHeadedAttentionAligned(nn.Module):
    def __init__(self, model_dim, head_count, dim_per_head=None, dropout=0.1, exclude_diagonal=False):

        super(MultiHeadedAttentionAligned, self).__init__()
    ...
    def forward(self, key, value, query, mask=None, enc_attn_cache=None, self_attn_cache=None, align_matrix_pad=None):
        ...
        # return all attn
        all_attn = attn.view(batch_size, head_count, query_len, key_len)[:, :, :, :].contiguous()

        # END CHECK
        return output, all_attn, [key_up, value_up]
